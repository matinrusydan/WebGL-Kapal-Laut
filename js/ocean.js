...
export const Ocean = {
  ...
  draw() {
    ...
    const baseW = 40.0 / wv.freq; 
    const maxS = 0.35; 
    const s = Math.min(wv.chop * 0.5, maxS);
    ...
    WebGL.gl.uniform3fv(u('uColor'), Config.wave[1].color); // Memaksa warna absolut (selalu menggunakan color level 1)
    WebGL.gl.uniform3fv(u('uCamPos'),State.camera.pos);
    WebGL.gl.uniform1f(u('uShininess'),280); WebGL.gl.uniform1f(u('uOpacity'),0.97);
